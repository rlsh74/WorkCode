...
# Version 2.0
# Each prediction for test dataset we make separately (in loop)
...
for ticker in tickers:
    try:
        ...
        #model = XGBClassifier(use_label_encoder=False, objective='reg:squarederror', random_state=1).fit(X_train, y_train)
        model = XGBRegressor(learning_rate=0.05, random_state=7).fit(X_train, y_train)
        ...
        df_result['Price'] = df_result['Close']
        df_result['Flag'] = np.where(df_result['Close'] < df_result['Pred'], 1, 0)
                
        #Get the model metrics
        #print(classification_report(y_test, pred))
        #print(model.feature_importances_)
        #df_result.to_excel('__rf_' + ticker+ '_' +str(K) + '_' + interval + '_signal.xlsx', index=False)
        
        #plt.figure(figsize=(15, 8))
        #plt.scatter(df_result.index, df_result['Close'], label='Close', color='green')
        #plt.scatter(df_result.index, df_result['Pred'], label='Model', color='red')
        #plt.legend(loc='upper left')
        #plt.title(ticker)
        
        # Here must be the backtester
        Signal_maker(df_result, 'Flag')

        res = df_result[['Date', 'Ticker', 'Close', 'BS', 'SS']]
        
        res.to_excel('__RF_' + ticker+ '_' +str(K) + '_' + interval + '_signal.xlsx', index=False)
...
#signals.to_excel('_All_signals_2.xlsx', index=False)

for ticker in tickers:
    try:
        df = pd.read_excel('__RF_' + ticker+ '_' +str(K) + '_' + interval + '_signal.xlsx')
