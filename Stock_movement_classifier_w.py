...
def Signal_chart(ticker, data):
    ...
    plt.plot(dat['Close'], label=ticker, alpha=0.65)
    plt.scatter(dat.index, dat['BS'], label='Buy', marker='^', color='green')
    plt.scatter(dat.index, dat['SS'], label='Sell', marker='v', color='red')
...
signals.to_excel(my_path + '__All_signals_w.xlsx', index=False)
